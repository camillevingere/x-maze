...
/*unsigned char attendreEvenementMult(int *touche[5], unsigned char *fenetre, unsigned char *quitter)
{
  SDL_Event event;
  *fenetre = 0;
  *quitter = 0;
  ...
  case SDL_KEYDOWN:
    switch (event.key.keysym.sym)
    {
      if (event.key.keysym.sym == SDLK_LEFT)
        *touche[0] = TOUCHE_GAUCHE;
      else
        *touche[0] = TOUCHE_AUTRE;

      if (event.key.keysym.sym == SDLK_RIGHT)
        *touche[1] = TOUCHE_DROITE;
      else
        *touche[1] = TOUCHE_AUTRE;

      if (event.key.keysym.sym == SDLK_UP)
        *touche[2] = TOUCHE_HAUT;
      else
        *touche[2] = TOUCHE_AUTRE;

      if (event.key.keysym.sym == SDLK_DOWN)
        *touche[3] = TOUCHE_BAS;
      else
        *touche[3] = TOUCHE_AUTRE;

      if (event.key.keysym.sym == SDLK_SPACE)
        *touche[4] = TOUCHE_TIR;
      else
        *touche[4] = TOUCHE_AUTRE;
    }
    break;
  case SDL_KEYUP:
    *touche = 0;
